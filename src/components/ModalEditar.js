...
const ModalEditar = ({ show, handleClose, handleEdit, user }) => {
  const [updatedUser, setUpdatedUser] = useState({ nombre: '', correo: '', contrasena: '' });
      ...
      <Modal.Body>
        <Form>
          ...
          <Form.Group controlId="formPassword" className="mt-3">
            ...
            <Form.Control
              type="password"
              value={updatedUser.contrasena}
              onChange={(e) => setUpdatedUser({ ...updatedUser, contrasena: e.target.value })}
            />
          </Form.Group>
          {error && <div className="text-danger mt-2">{error}</div>}
          <Button variant="success" className="mt-3 w-100" onClick={handleSubmit}>
            Guardar Cambios
          </Button>
        </Form>
      </Modal.Body>
